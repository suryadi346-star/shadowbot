...
If you discover a security vulnerability in shadowbot, please report it by:
...
chmod 600 ~/.shadowbot/config.json

# ❌ Bad: Hardcoding keys in code or committing them
```

**Recommendations:**
- Store API keys in `~/.shadowbot/config.json` with file permissions set to `0600`
...
- Config files and API keys (`~/.shadowbot/config.json`) → **hidden** (masked by tmpfs)
...
- ✅ Run shadowbot with a dedicated user account
...
- Keep authentication data in `~/.shadowbot/whatsapp-auth` secure (mode 0700)
...
pip install --upgrade shadowbot-ai
```
...
- Subscribe to security advisories for shadowbot and its dependencies
...
2. **Use a Dedicated User**
   ```bash
   sudo useradd -m -s /bin/bash shadowbot
   sudo -u shadowbot shadowbot gateway
   ```
...
3. **Set Proper Permissions**
   ...
   chmod 700 ~/.shadowbot
   chmod 600 ~/.shadowbot/config.json
   chmod 700 ~/.shadowbot/whatsapp-auth
   ```

4. **Enable Logging**
   ```bash
   # Configure log monitoring
   tail -f ~/.shadowbot/logs/shadowbot.log
   ```
...
- **Chat history is stored locally** - protect the `~/.shadowbot` directory
...
2. **Review logs for unauthorized access**
   ```bash
   grep "Access denied" ~/.shadowbot/logs/shadowbot.log
   ```
...
- GitHub Security Advisories: https://github.com/suryadi346-star/shadowbot/security/advisories
- Release Notes: https://github.com/suryadi346-star/shadowbot/releases
