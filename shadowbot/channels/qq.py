...
- Publish to Shadowbot bus via BaseChannel._handle_message()
...
from shadowbot.security.network import validate_url_target

try:
    from shadowbot.config.paths import get_media_dir
...
def _make_bot_class(channel: QQChannel) -> type[botpy.Client]:
    ...
    class _Bot(botpy.Client):
        def __init__(self):
            # Disable botpy's file log — shadowbot uses loguru; default "botpy.log" fails on read-only fs
...
class QQConfig(Base):
    ...
    # Optional: directory to save inbound attachments. If empty, use shadowbot get_media_dir("qq").
    media_dir: str = ""
    ...
    def _init_media_root(self) -> Path:
        ...
        if self.config.media_dir:
            ...
            try:
                ...
                root = Path.home() / ".shadowbot" / "media" / "qq"
        else:
            root = Path.home() / ".shadowbot" / "media" / "qq"
