...
echo "shadowbot line count"
echo "=================="
echo ""

echo "Core runtime"
echo "------------"
core_agent=$(count_top_level_py_lines "shadowbot/agent")
core_bus=$(count_top_level_py_lines "shadowbot/bus")
core_config=$(count_top_level_py_lines "shadowbot/config")
core_cron=$(count_top_level_py_lines "shadowbot/cron")
core_heartbeat=$(count_top_level_py_lines "shadowbot/heartbeat")
core_session=$(count_top_level_py_lines "shadowbot/session")
...
echo "----------------"
extra_tools=$(count_recursive_py_lines "shadowbot/agent/tools")
extra_skills=$(count_skill_lines "shadowbot/skills")
extra_api=$(count_recursive_py_lines "shadowbot/api")
extra_cli=$(count_recursive_py_lines "shadowbot/cli")
extra_channels=$(count_recursive_py_lines "shadowbot/channels")
extra_utils=$(count_recursive_py_lines "shadowbot/utils")
...
echo "Notes"
echo "-----"
echo "  - agent/ only counts top-level Python files under shadowbot/agent"
echo "  - tools/ is counted separately from shadowbot/agent/tools"
echo "  - skills/ counts .md, .py, and .sh files"
echo "  - not included here: command/, providers/, security/, templates/, shadowbot.py, root files"
