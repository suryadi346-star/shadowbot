...
try:
    from shadowbot.channels import dingtalk
...
import shadowbot.channels.dingtalk as dingtalk_module
from shadowbot.channels.dingtalk import DingTalkChannel, ShadowbotDingTalkHandler
from shadowbot.channels.dingtalk import DingTalkConfig
...
async def test_group_send_uses_group_messages_api() -> None:
    ...
    ok = await channel._send_batch_message(
        ...
        "group:conv123",
        "sampleMarkdown",
        {"text": "hello", "title": "Shadowbot Reply"},
    )
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = ShadowbotDingTalkHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/shadowbot_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/shadowbot_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "shadowbot.config.paths.get_media_dir",
