...
from shadowbot.channels.matrix import _build_matrix_text_content

import shadowbot.channels.matrix as matrix_module
from shadowbot.bus.events import OutboundMessage
from shadowbot.bus.queue import MessageBus
from shadowbot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from shadowbot.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("shadowbot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "shadowbot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("shadowbot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "shadowbot.channels.matrix.asyncio.create_task", _fake_create_task
    )
