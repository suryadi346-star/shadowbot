...
from shadowbot.agent.tools.web import WebSearchTool
...
async def test_brave_search(monkeypatch):
    async def mock_get(self, url, **kw):
        ...
        return _response(json={
            "web": {"results": [{"title": "ShadowBot", "url": "https://example.com", "description": "AI assistant"}]}
        })

    monkeypatch.setattr(httpx.AsyncClient, "get", mock_get)
    tool = _tool(provider="brave", api_key="brave-key")
    result = await tool.execute(query="shadowbot", count=1)
    assert "ShadowBot" in result
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("shadowbot.agent.tools.web.DDGS", MockDDGS, raising=False)
    import shadowbot.agent.tools.web as web_mod
