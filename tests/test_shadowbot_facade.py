"""Tests for the Shadowbot programmatic facade."""
...
from shadowbot.shadowbot import Shadowbot, RunResult
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        Shadowbot.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = Shadowbot.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("shadowbot.config.loader.load_config") as mock_load, \
         patch("shadowbot.shadowbot._make_provider") as mock_prov:
        mock_load.return_value = Config()
        mock_prov.return_value = MagicMock()
        mock_prov.return_value.get_default_model.return_value = "test"
        mock_prov.return_value.generation.max_tokens = 4096
        Shadowbot.from_config()
...
def test_workspace_override(tmp_path):
    ...
    bot = Shadowbot.from_config(config_path, workspace=custom_ws)
    assert bot._loop.workspace == custom_ws


def test_sdk_make_provider_uses_github_copilot_backend():
    from shadowbot.config.schema import Config
    from shadowbot.shadowbot import _make_provider
...
def test_import_from_top_level():
    from shadowbot import Shadowbot as N, RunResult as R
    assert N is Shadowbot
