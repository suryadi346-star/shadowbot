...
Use shadowbot programmatically — load config, run the agent, get results.
...
### `Shadowbot.from_config(config_path?, *, workspace?)`

Create a `Shadowbot` from a config file.

| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config_path` | `str \| Path \| None` | `None` | Path to `config.json`. Defaults to `~/.shadowbot/config.json`. |
...
async def main():
    bot = Shadowbot.from_config(workspace="/my/project")
