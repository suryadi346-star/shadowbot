...
Build a custom shadowbot channel in three steps: subclass, package, install.

> **Note:** We recommend developing channel plugins against a source checkout of shadowbot (`pip install -e .`) rather than a PyPI release, so you always have access to the latest base-channel features and APIs.

## How It Works

shadowbot discovers channel plugins via Python [entry points](https://packaging.python.org/en/latest/specifications/entry-points/). When `shadowbot gateway` starts, it scans:

1. Built-in channels in `shadowbot/channels/`
2. External packages registered under the `shadowbot.channels` entry point group
...
```
shadowbot-channel-webhook/
├── shadowbot_channel_webhook/
...
```python
# shadowbot_channel_webhook/__init__.py
from shadowbot_channel_webhook.channel import WebhookChannel

__all__ = ["WebhookChannel"]
```

```python
# shadowbot_channel_webhook/channel.py
import asyncio
...
# pyproject.toml
[project]
name = "shadowbot-channel-webhook"
version = "0.1.0"
dependencies = ["shadowbot", "aiohttp"]

[project.entry-points."shadowbot.channels"]
webhook = "shadowbot_channel_webhook:WebhookChannel"
...
```bash
pip install -e .
shadowbot plugins list      # verify "Webhook" shows as "plugin"
shadowbot onboard           # auto-adds default config for detected plugins
```

Edit `~/.shadowbot/config.json`:
...
```bash
shadowbot channels login <channel_name>
shadowbot channels login <channel_name> --force  # re-authenticate
```
...
| `default_config()` (classmethod) | Returns default config dict for `shadowbot onboard`. Override to declare your fields. |
...
Built-in channels use Pydantic config models (subclassing `Base` from `shadowbot.config.schema`). Plugin channels **must do the same**.

### Pattern

1. Define a Pydantic model inheriting from `shadowbot.config.schema.Base`:
...
Override `default_config()` so `shadowbot onboard` auto-populates `config.json`:
...
| What | Format | Example |
|------|--------|---------|
| PyPI package | `shadowbot-channel-{name}` | `shadowbot-channel-webhook` |
| Entry point key | `{name}` | `webhook` |
| Config section | `channels.{name}` | `channels.webhook` |
| Python package | `shadowbot_channel_{name}` | `shadowbot_channel_webhook` |

## Local Development

```bash
git clone https://github.com/you/shadowbot-channel-webhook
cd shadowbot-channel-webhook
pip install -e .
shadowbot plugins list    # should show "Webhook" as "plugin"
shadowbot gateway         # test end-to-end
```

## Verify

```bash
$ shadowbot plugins list
